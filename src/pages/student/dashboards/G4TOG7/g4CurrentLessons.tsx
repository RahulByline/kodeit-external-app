...
const getCacheKey = (key: string) => `g4_current_lessons_${key}`;
...
const CurrentLessons: React.FC = () => {
  ...
  const loadLessonsWithCache = useCallback(async () => {
    ...
    try {
      ...
      const lessonsCacheKey = getCacheKey(`${CACHE_KEYS.CURRENT_LESSONS}_${currentUser.id}`);
      const coursesCacheKey = getCacheKey(`${CACHE_KEYS.USER_COURSES}_${currentUser.id}`);
        ...
        try {
          ...
          const courseLessonPromises = enrolledCourses.map(async (course) => {
            try {
              ...
              const courseLessons = await enhancedMoodleService.getCourseContents(course.id.toString());
  ...
  const preloadLessonData = useCallback(async (lesson: Lesson) => {
    ...
    try {
      const lessonData = await enhancedMoodleService.getCourseContents(lesson.courseId);
