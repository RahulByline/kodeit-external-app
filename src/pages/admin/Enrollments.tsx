...
const Enrollments: React.FC = () => {
  ...
  const fetchEnrollmentData = async () => {
    try {
      ...
      // Fetch all users and courses for enrollment data with proper error handling
      let users: any[] = [];
      let courses: any[] = [];
      let categories: any[] = [];
      
      try {
        console.log('📡 Making API calls to Moodle for enrollment data...');
        ...
        console.log('📊 Enrollment API Results:', {
          users: usersResult,
        ...
        console.log(`✅ Successfully fetched ${users.length} users, ${courses.length} courses, and ${categories.length} categories`);
      } catch (apiError) {
      ...
      // Filter students with proper async handling
      const students = [];
      for (const user of users) {
        try {
          const role = await moodleService.detectUserRoleEnhanced(user.username || '', user, user.roles || []);
          if (role === 'student') {
            students.push(user);
          }
        } catch (roleError) {
          console.warn('⚠️ Role detection failed for user:', user.username, roleError);
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {courseEnrollments.length > 0 ? (
                ...
                <div className="text-center py-8 text-gray-500">
                  <p>No course enrollments to display.</p>
                </div>
              )}
            </div>
          </div>
